    ...
    for (const auto& command : commands_) {
        if (command.command == "Stop") {
            ...
            for (const auto& stop_name : stops) {
                stop_pointers.push_back(catalogue.GetStop(stop_name));
            }
            
            bool is_circle = (command.description.find('>') != std::string::npos);
            catalogue.AddRoute(command.id, stop_pointers, is_circle);
        }
    }
}
